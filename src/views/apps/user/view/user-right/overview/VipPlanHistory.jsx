...
// Service Imports
import { getUserVipPlanHistory } from '@/services/ApiService'
...
const VipPlanHistory = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setLoading(true)
            try {
                const { ok, result } = await getUserVipPlanHistory({ 
                    id, 
                    page: page + 1, 
                    limit: rowsPerPage 
                })
  ...
  return (
    <Grid container spacing={6}>
      <Grid size={{ xs: 12 }}>
        <Card>
            <CardHeader title='VIP Plan History' />
                                ...
                                {data.length > 0 ? (
                                    data.map((row, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell>{row.day || row.planId?.day || '0'}</TableCell>
                                            <TableCell>
                                                <Chip 
